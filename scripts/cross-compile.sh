...
# This assumes main.go is in the root of the project.
PACKAGE_PATH="."
...
GOOS=linux GOARCH=amd64 go build -ldflags="-s -w -X main.Version=${VERSION}" -o ${OUTPUT_DIR}/archguardian-linux-amd64 ${PACKAGE_PATH}

# --- Compile for Windows (amd64) ---
echo "Compiling for Windows (amd64)..."
GOOS=windows GOARCH=amd64 go build -ldflags="-s -w -X main.Version=${VERSION}" -o ${OUTPUT_DIR}/archguardian-windows-amd64.exe ${PACKAGE_PATH}

# --- Compile for macOS (Intel/amd64) ---
echo "Compiling for macOS (amd64)..."
GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w -X main.Version=${VERSION}" -o ${OUTPUT_DIR}/archguardian-darwin-amd64 ${PACKAGE_PATH}

# --- Compile for macOS (Apple Silicon/arm64) ---
echo "Compiling for macOS (arm64)..."
GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w -X main.Version=${VERSION}" -o ${OUTPUT_DIR}/archguardian-darwin-arm64 ${PACKAGE_PATH}
